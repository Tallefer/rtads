...
#define FILE_TYPE_DATA    4 /* файл общих данных (используется для fopen()) */
...
specialWords
   'of'='для'='против'='типа'='из'='под'='от'='на'='в'='с',/* используется в */
...
// Даёт окончания прилагатльных "ие", "ой", "ая", "ое"
oyay: function( obj )
{
  local gen = obj.gender;
  if (gen=3) "ие";
  ...
  {
  if (obj.gender=1) "ой"; else
  if (obj.gender=2) "ая"; else "ое";
  }
}
...
class chairitem: fixeditem, nestedroom, surface
    ...
    verDoSiton(actor) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> уже на "; self.mdesc; "! ";
        }
    }
    doSiton(actor) =
    {
...
class thing: object
    ...
    */
    ...
    verDoWear(actor) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> не "; glok(actor,1,1,'мож');" надеть "; self.vdesc; "! ";
    }
    verDoTake(actor) =
    {
    ...
    cantReach(actor) =
    {
        ...
        if (self.location = nil)
        {
            if (actor.location.location)
                ...
                "<<ZAG(actor,&ddesc)>> не достать до <<self.ritobjdesc>>. ";
            return;
        }
    ...
    verDoDrop(actor) =
    {
        if (not actor.isCarrying(self))
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не ";glok(actor,1,2,'нес'); " "; self.rdesc; "! ";
        }
    ...
    verIoPutIn(actor) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> не ";glok(actor,1,1,'мож');" положить что-либо в "; self.vdesc; ". ";
    }
    circularMessage(io) =
    {
        local cont, prep;
    ...
    verDoPutIn(actor, io) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не ";glok(actor,1,1,'мож'); " положить "; self.vdesc; " в <<self.itselfdesc>>! ";
        }
    ...
    verDoPutOn(actor, io) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не ";glok(actor,1,1,'мож'); " положить ";
    ...
    verIoPlugIn(actor) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> не ";glok(actor,1,1,'мож'); " подключить что-либо к "; self.ddesc; ". ";
    }
    verDoPlugIn(actor, io) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> подключить "; self.vdesc; " к чему-либо. ";
    }
    ...
    verDoLookthru(actor) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> ничего не "; glok(actor,2,1,'вид'); " через "; self.vdesc; ". ";
    }
    ...
    verDoScrew(actor) = { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'зна')>> как это сделать. "; }
    verDoScrewWith(actor, iobj) = { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'зна')>> как это сделать. "; }
    verIoScrewWith(actor)={"<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> так использовать <<self.vdesc>>. "; }

    verDoUnscrew(actor) = { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'зна')>> как это сделать. "; }
    verDoUnscrewWith(actor, iobj) = { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'зна')>> как это сделать. "; }
    verIoUnscrewWith(actor) =
        { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> так использовать <<self.vdesc>>. "; }
    ...
    verDoUnboard(actor) =
    {
        if (actor.location <> self)
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не <<self.statusPrep>> <<self.mdesc>>! ";
        }
        else if (self.location = nil)
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> сойти с <<self.rdesc>>! ";
        }
    }
    doUnboard(actor) =
    {
    ...
    verDoThrowTo(actor, io) =
    {
        if (not actor.isCarrying(self))
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'име')>> "; self.rdesc; ". ";
        }
        else
            self.verifyRemove(actor);
    }
    doThrowTo(actor, io) =
    {
        "<<ZAG(parserGetMe(),&sdesc)>> промахнул"; saas(actor);". ";
    ...
    verDoSearchIn( actor, iobj ) = { "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> искать в <<self.mdesc>>. "; }
...
class lightsource: item
    ...
    doTurnon(actor) =
    {
        ...
        self.isActive := true;
        self.islit := true;
        "<<ZAG(parserGetMe(),&sdesc)>> включ<<ok(parserGetMe(), 'или', 'ил', 'ило', 'ила')>> <<self.vdesc>>";
...
class movableActor: qcontainer // A character in the game
    ...
    doAskAbout(actor, iobj) =
    {
        ...
        tot := length(lst);
        if ((tot = 1 and (find(['этом' 'них' 'нем' 'ней'], lst[1]) <> nil))
            or tot = 0)
        {
    ...
    {
        if (actor = self)
            "Оригинально! \"Я всегда делюсь с собой\". ";
    }
    ioGiveTo(actor, dobj) =
    {
...
class basicMe: Actor, floatingItem
    ...
    noun = 'me' 'myself' 'меня' 'мной' 'мной#t' 'мною' 'мною#t' 'собой' 'собой#t' 'собою' 'собою#t' 'мне' 'мне#d' 'себе' 'себе#d' 'я' 
    lico=2
    sdesc = "ты"
    ldesc = "<<ZAG(parserGetMe(),&sdesc)>> <<glok(parserGetMe(),2,2,'выгляд')>> также как и всегда. "
    rdesc = "тебя"
    vdesc = "тебя"
    ddesc = "тебе"
    tdesc = "тобою"
...
class clothingItem: item
    ...
    verDoWear(actor) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> уже ";glok(actor,2,1,'нос');" ";self.vdesc; "! ";
        }
    }
    doWear(actor) =
    {
    ...
    verDoUnwear(actor) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не "; glok(actor,2,1,'нос');" ";self.vdesc; ". ";
        }
    }
    doUnwear(actor) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> <<glok(actor,1,2,'мож')>> бросить <<self.vdesc>>, но <<actor.fmtYour>> руки
...
class vehicle: item, nestedroom
    ...
    verDoBoard(actor) =
    {
        if (actor.location = self)
        {
            "<<ZAG(parserGetMe(),&sdesc)>> уже <<self.statusPrep>> <<self.mdesc>>! ";
        }
    ...
    dobjGen(a, v, i, p) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&ddesc)>> сначала придётся слезть <<self.outOfPrep>> <<
                self.rdesc>>. ";
            exit;
        }
    }
...
class container: item
    ...
    ioPutIn(actor, dobj) =
    {
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> не <<glok(actor,1,1,'мож')>> всунуть <<dobj.vdesc>> в "; self.vdesc; ". ";
        }
        else
        {
...
class transparentItem: item
    ...
    {
        if (self.contentsVisible and itemcnt(self.contents) <> 0)
            "В <<self.mdesc>> %You% <<glok(parserGetMe(),2,1,'вид')>> <<listcont(self)>>. ";
        else
            "В <<self.mdesc>> ничего нет. ";
    }
;
...
// Указатель режима работы глагола с творительным и дательным падежами
padezh_type = {
 local result;
 if (verbinfo(self,toPrep)) result:=2;
 if (verbinfo(self,withPrep)) 
  {
    if (result) result:=3; else result:=1;
  }
 return result;
}
...
inspectVerb: deepverb
    verb = 'изучить' 'изучи' 'рассмотреть' 'рассмотри' 'осмотреть' 'осмотри' 'изуч' 'исследовать' 'исследуй' 'оглядеть' 'огляди' 'обследовать' 'обследуй' 'ис'
    sdesc = "изучить"
    doAction = 'Inspect'
    validDo(actor, obj, seqno) =
    {
...
askVerb: deepverb
    verb = 'спросить' 'расспросить' 'спроси' 'расспроси'
    sdesc = "спросить"
    vopr = "Кого "
...
followVerb: deepverb
    sdesc = "следовать"
    vopr = "За кем "
    verb = 'следовать' 'следуй' 'следовать за' 'следуй за' 'преследовать' 'преследуй' 'следить за' 'следи за'
           'идти за' 'иди за'
    doAction = 'Follow'
;
digVerb: deepverb
...
jumpVerb: deepverb   //Добавил ГрАнд
    verb = 'прыгнуть' 'перепрыгнуть' 'перепрыгни' 'спрыгнуть' 'спрыгни' 'прыгать' 'подпрыгнуть' 'подпрыгни' 'прыгни' 'прыгай'
    sdesc = "прыгнуть"
    doAction = 'Jump'
    action(actor) = { "Гоп! "; }
;
...
;
openVerb: deepverb
    verb = 'открыть' 'открой' 'отворить' 'отвори' 'раскрыть' 'раскрой' 'распахнуть' 'распахни' 'отк' 'откр'  'вскрыть' 'вскрой'
    sdesc = "открыть"
    doAction = 'Open'
...
;
lookInVerb: deepverb
    verb = 'смотреть в' 'смотри в' 'посмотреть в' 'посмотри в' 'заглянуть в' 'загляни в' 'глядеть в'
    'глянь в' 'взглянуть в' 'взгляни в' 'г в' 'см в' 'посм в' 'посм на'
    'смотреть внутрь' 'смотри внутрь' 'посмотреть внутрь' 'посмотри внутрь' 'заглянуть внутрь' 'загляни внутрь' 'глядеть внутрь' 'глянь внутрь' 'взглянуть внутрь' 'взгляни внутрь' 'г внутрь' 'см внутрь'
           'см на' 'г на' 'смотреть на' 'смотри на' 'посмотреть на' 'посмотри на' 'взглянуть на' 'взгляни на' 'глядеть на' 'глянь на'
...
;
eatVerb: deepverb
    verb = 'есть' 'жрать' 'кушать' 'съесть' 'откусить' 'поесть' 'сожрать' 'слопать' 'жевать' 'проглотить' 'попробовать'
           'пробовать' 'ешь' 'жри' 'кушай' 'съешь' 'откуси' 'поешь' 'сожри' 'слопай' 'жуй' 'проглоти' 'попробуй' 'пробуй'
...
;
readVerb: deepverb
    verb = 'читать' 'прочесть' 'прочесть с' 'прочитать' 'зачитать' 'читай' 'прочитай' 'прочти' 'прочитай с' 'прочти с' 'зачитай' 'прочитать с'  
    'почитать' 'почитай'
    sdesc = "читать"
    doAction = 'Read'
;
...
helloVerb: deepverb
    verb = 'привет' 'здравствуйте' 'здороваться' 'поздороваться' 'приветствовать'
           'здравствуй' 'поздоровайся' 'приветствуй' 'хай' 'здорово'
    sdesc = "привет"
    action(actor) =
    {
        "Как дела?\n";
...
;
talkVerb: deepverb
    verb = 'поговорить' 'поговори' 'поболтать' 'поболтай' 'говорить' 'говори' 'разговаривать' 'беседовать' 'беседуй' 'побеседовать' 'побеседуй'
    'поговорить с' 'поговори с' 'поболтать с' 'поболтай с' 'говорить с'
    'поговорить со' 'поговори со' 'поболтать со' 'поболтай со' 'говорить со'
...
unfastenVerb: deepverb
    verb = 'расстегнуть' 'расстегнись' 'отстегнуть' 'отстегни' 'отстегнуться' 'отстегнись' 'расстегнуться' 'расстегнись'
    sdesc = "расстегнуть"
    doAction = 'Unfasten'
;
unplugVerb: deepverb
...
lookUnderVerb: deepverb
    verb = 'смотреть под' 'посмотреть под' 'см под' 'г под' 'смотри под' 'посмотри под' 'заглянуть под' 'загляни под' 'взглянуть под' 'взгляни под'
    vopr = "Под что "
    sdesc = "посмотреть"
    doAction = 'Lookunder'
;
...
sleepVerb: darkVerb
    ...
    {
        if (actor.cantSleep)
            "<<ZAG(parserGetMe(),&sdesc)>> слишком озабочен<<yao(actor)>> выживанием, чтобы спать. ";
        else if (global.awakeTime+1 < global.sleepTime)
            "<<ZAG(parserGetMe(),&sdesc)>> не устал<<iao(actor)>>. ";
        ...
        {
            "<<ZAG(parserGetMe(),&sdesc)>> быстро уплыл<<iao(actor)>> в страну снов...\b";
            goToSleep();
        }
    }
...
'прикоснуться' 'прикоснись' 'коснись' 'касаться' 'коснись' 'ощупать' 'ощупай' 'пощупать' 'погладить' 'погладь' 
'дотронуться до' 'дотронься до'
    sdesc = "трогать"
    doAction = 'Touch'
;
moveNVerb: deepverb
...
neVerb: travelVerb
    action(actor) = { actor.travelTo(self.travelDir(actor)); }
    verb = 'идина северо-восток' 'северо-восток' 'на северо-восток' 'св' 'на св' 'идина св' 'идти северо-восток' 'идина северо-восток' 'идина св' 'иди северо-восток'
    sdesc = "идти на северовосток"
    travelDir(actor) = { return actor.location.(actor.neProp); }
;
nwVerb: travelVerb
    action(actor) = { actor.travelTo(self.travelDir(actor)); }
    verb = 'идина северозапад' 'идина сз' 'северо-запад' 'сз' 'на сз' 'идина сз' 'идти сз' 'идти северо-запад' 'идина северо-запад' 'идина сз' 'иди северо-запад'
    sdesc = "идти на северозапад"
    travelDir(actor) = { return actor.location.(actor.nwProp); }
;
seVerb: travelVerb
    action(actor) = { actor.travelTo(self.travelDir(actor)); }
    verb = 'юго-восток' 'идина юго-восток' 'идина юв' 'юв' 'на юв' 'идина юв'  'идти юв' 'идина юго-восток' 'идина юв' 'идти юго-восток' 'иди юго-восток'
    sdesc = "идти на юговосток"
    travelDir(actor) = { return actor.location.(actor.seProp); }
;
swVerb: travelVerb
    action(actor) = { actor.travelTo(self.travelDir(actor)); }
    verb = 'юз' 'идина юго-запад' 'юго-запад' 'на юз' 'на юго-запад' 'идина юз' 'идти юз' 'идина юго-запад' 'иди юго-запад' 'идина юз'
...
;
withPrep: Prep
    preposition = 'with' 'спомощью' 'припомощи'
    sdesc = "с помощью"
;
toPrep: Prep
    preposition = 'to' 'к' 'ко'
...
preparseCmd: function(lst)
{
   ...
   while (changed && loopnum<maxloops )
   {
    ...
    for (i := 1, len := length(lst) ; i <= len ; ++i)
    {
      ...
      if ( (lst[i]=',' or lst[i]='.') and (length(lst)>i) )
      {
        objs:=parserDictLookup([(lst[i+1])], [PRSTYP_VERB]);
      ...
      if (firstword)
      {
       ...
       else {lastVerbNum:=i; lastVerbNumInNew:=length(newlst)+1;}
      }
       ...
       if (!firstword)  
         ...
         objs:=parserDictLookup([(cur)], [PRSTYP_PREP]);
          ...
          if (objs!=[]) 
           {
            objs:=parserDictLookup([(lst[lastVerbNum]+' '+cur)], [PRSTYP_VERB]);

	    // если да, и других предлогов после глагола не было

            if (objs!=[] && (!lastPrepInNew || lastPrepInNew<lastVerbNumInNew) && (!lastPrepInNew && lastPrepInNew!=lastVerbNumInNew+1)) 
            {
        ...
        if (wasnoun) 
        {
          ...
          if (objs!=[]) 
          {
            //local nobjs := parserDictLookup([(lst[i-1])], [PRSTYP_NOUN]);
       ...
       if (global.glpad=1 or global.glpad=3)
       {
        ...
        if (global.glpad=2 or global.glpad=3)
        {
         ...
         if (objs != [])
          {
           ...
           newlst += 'to'; 
           global.glpad:=objs[1].padezh_type;
          }
         }
         changed := true;
        }
  ...
  if (lastVerbNum<>nil && lastPrepInNew<>nil && (i=length(lst) || firstword))
...
/* Функция позволяет парсеру понимать слово "себя" */
...
{       
 ...
 if (wordlist[current_index]='себя') return [(current_index+1), parserGetObj(PO_ACTOR), 0];
 else if (wordlist[current_index]='себе') return [(current_index+1), parserGetObj(PO_ACTOR), 0];  

// алгоритм проверяет, может ли пара "сущ + прил" быть вариантом "прил + сущ"
 if (current_index>1)
  {
...

   {
     ...
     if (((typelist[i] & PRSTYP_ADJ) != 0))
     {
      ...
      if (objs!=[]) objs:=intersect( objs, tobjs ); else objs:=tobjs;
