/* Copyright (c) 2002-2008 by Гранкин Андрей П.  All Rights Reserved. 
*                            Release 25			
*/
...
{
    "Я не знаю как << v.sdesc >> "; if (v.pred) {v.pred.sdesc;" ";}
      if (d)
      {
      if (v.pred!=nil)
       {if  (v.pred=withPrep or v.pred=toPrep or v.pred=goonPrep) d.ddesc; else  {d.vdesc;". ";} }
      else {d.vdesc; ". ";}
      }
      else
{
 ...
 if (p) 
 {
 if (dToS(p,&sdesc)!='к') {p.sdesc;" ";}
 if (p=toPrep or p=goonPrep) i.ddesc; else
 if (p=onPrep) i.mdesc; else
